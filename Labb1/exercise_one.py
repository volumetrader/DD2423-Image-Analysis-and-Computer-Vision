...
def fftwave(u, v, sz = 128):
    ...
    if v < sz/2:
        ...
        vc = v - sz
    wavelength = sz / math.sqrt(uc**2 + vc**2)  # Replace by correct expression
    amplitude = math.sqrt(np.sum(np.square(np.real(Fhat))) + np.sum(np.square(np.imag(Fhat[uc, vc]))))  # Replace by correct expression
